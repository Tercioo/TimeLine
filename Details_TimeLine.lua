...
local TimeLine = Details:NewPluginObject ("Details_TimeLine", DETAILSPLUGIN_ALWAYSENABLED)
...
local function CreatePluginFrames()
	...
	local DetailsFrameWork = Details.gump

	local framework = _G.Details:GetFramework()
	...
	local timeLineFrame = CreateFrame ("Button", nil, TimeLineFrame, "UIPanelCloseButton")
	timeLineFrame:SetWidth (20)
	timeLineFrame:SetHeight (20)
	timeLineFrame:SetPoint ("TOPRIGHT", TimeLineFrame, "TOPRIGHT", -2, -3)
	timeLineFrame:SetFrameLevel (TimeLineFrame:GetFrameLevel()+1)
	timeLineFrame:GetNormalTexture():SetDesaturated (true)
	timeLineFrame:SetAlpha (1)
...
--window title
	local titleLabel = Details.gump:NewLabel (titleBar, titleBar, nil, "titulo", "Details! Time Line", "GameFontHighlightLeft", 12, {227/255, 186/255, 4/255})
	...
	TimeLine.window_open = false	--rows
	TimeLine.rows = {}
	...
	local SetPlayer = function (row, playerName, playerDataTable, elapsedTime, pixelPerSecond, deaths)
		...
		if (current_type == type_cooldown and playerDataTable) then
		...
		--> showing debuffs
		elseif (current_type == type_debuff and playerDataTable) then
		...
		elseif (current_type == type_enemy_spells and playerDataTable) then
			local o = 1
		...
		elseif (current_type == type_boss_spells and playerDataTable) then
			local o = 1
			for index, spellTable in ipairs (playerDataTable) do
				local combatTime, sourceName, spellID, token, targetName = unpack (spellTable)
...
local build_options_panel = function()
	...
	Details.gump:BuildMenu (options_frame, menu, 15, -65, 260, true, options_text_template, options_dropdown_template, options_switch_template, true, options_slider_template, options_button_template)
	...
	elseif (event == "ADDON_LOADED") then
		...
		if (AddonName == "Details_TimeLine") then
			...
			if (_G.Details) then
				...
				_G.Details:RegisterEvent (TimeLine, "COMBAT_PLAYER_ENTER")
				_G.Details:RegisterEvent (TimeLine, "COMBAT_PLAYER_LEAVE")
				_G.Details:RegisterEvent (TimeLine, "DETAILS_DATA_RESET")

				_G.Details:InstallHook (DETAILS_HOOK_COOLDOWN, TimeLine.OnCooldown)
